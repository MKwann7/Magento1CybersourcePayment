...
class CustomExtensions_Cybersource_Model_Cards
{
    ...
    {
        Mage::log('Payment Card Key = '.json_encode($this->_cards),null,'mwz_payment_saving_testing.log');
        $_cards = array();
        foreach(array_keys($this->_cards) as $key) {
            $_cards[$key] = $this->getCard($key);
        }
        Mage::log('Payment Card Key = '.json_encode($_cards),null,'mwz_payment_saving_testing.log');
        return $_cards;
    }
